...
// customerBuild should receive the following parameters: config, options.
...
class Factory {
  ...
  static massBuild(quantity, options) {
    console.log(
      `Building ${quantity} ${options.color} ${options.trim} ${options.model}'s`
    );
  }
  static customerBuild(config, options) {
    console.log(
      `Building one ${config.color} ${config.trim} ${config.model} with the following options: ${options}`
    );
  }
}
...
class Car extends Factory {
  constructor({ model, color, transmission, trim, wheelstrim, audio, seatstrim, moonroof, enginetype, doors }) {
    super();
    this.model = model;
    this.doors = doors;
    this.color = color;
...
// The following properties must be specified: model (mazda3), color (red), enginetype (hybrid), transmission (automatic), trim (touring), wheelstrim (base), audio (premium), seats (leather), and moonroof (true)
...
let customMazda3 = new Car({ model: "mazda3", doors: 4, color: "yellow", enginetype: "hybrid", transmission: "automatic", trim: "touring", wheelstrim: "base", audio: "premium", seatstrim: "leather", moonroof: true });

Factory.customerBuild(customMazda3, [
  "weather package",
  "satellite radio",
  "rear spoiler"
]);
...
Factory.customerBuild(customMiataRf, [
  "hid headlights",
  "sports suspension",
...
// Write your code below:
let customTrailBlazer = new Truck({
  model: "Trail Blazer",
  color: "red",
  trim: "Sport",
  enginesize: 8,
...
Factory.customerBuild(
  customTrailBlazer, ["seat warmers", "tinted windows", "fog lamps"]);
